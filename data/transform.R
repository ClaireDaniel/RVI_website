...
data_2011 <- data |> select(postcode, suburbs, ends_with("_2011"), ends_with("_common")) |> mutate(year = 2011, postcode = as.character(postcode)) |> rename_with(~str_remove(., '_2011$')) |> rename_with(~str_remove(., '_common$'))
data_2016 <- data |> select(postcode, suburbs, ends_with("_2016"), ends_with("_common")) |> mutate(year = 2016, postcode = as.character(postcode)) |> rename_with(~str_remove(., '_2016$')) |> rename_with(~str_remove(., '_common$'))
data_2021 <- data |> select(postcode, suburbs, ends_with("_2021"), ends_with("_common")) |> mutate(year = 2021, postcode = as.character(postcode)) |> rename_with(~str_remove(., '_2021$')) |> rename_with(~str_remove(., '_common$'))

geoms_2011 <- st_read("2011","2011") |> select(postcode = POA_CODE) |> filter(str_detect(postcode,'^2|^4')) |> mutate(state = ifelse(str_detect(postcode,'^2'),'nsw','qld'))
geoms_2016 <- st_read("2016","2016") |> select(postcode = POA_CODE16) |> filter(str_detect(postcode,'^2|^4')) |> mutate(state = ifelse(str_detect(postcode,'^2'),'nsw','qld'))
geoms_2021 <- st_read("2021","2021") |> select(postcode = POA_CODE21) |> filter(str_detect(postcode,'^2|^4')) |> mutate(state = ifelse(str_detect(postcode,'^2'),'nsw','qld'))
...
joined_2011 <- geoms_2011 |> full_join(data_2011) |> st_as_sf() |> filter(!st_is_empty(geometry)) |> st_transform("epsg:4326")
