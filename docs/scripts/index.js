...
let THEMES = [
  ...
  { "name": "People Indicators (Renters)",
    "items": [
      ...
      { id: 38, display: true,  label: "Recent Renters",          tooltip: "", legend: [0,0.05,0.1,0.15,0.2],    legend_format: d => d.toLocaleString(undefined,{style: 'percent', minimumfractiondigits:1, maximumfractiondigits:1}), color: c => c == 0 ? [200, 200, 200, 0.5] : chroma.scale('OrRd').domain([0,0.3]).classes(5)(c).rgb(),   value: d => d.recrenters/d.total_renters, format: d => `${d.recrenters} (${(d.recrenters/d.total_renters).toLocaleString(undefined,{style: 'percent', minimumfractiondigits:2, maximumfractiondigits:2})})` },
